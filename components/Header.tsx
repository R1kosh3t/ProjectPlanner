  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-y-4 gap-x-6">
      ...
      <div className="flex flex-wrap items-center justify-start sm:justify-end gap-3 w-full sm:w-auto">
          ...
          <button onClick={onProfileClick} className="flex items-center gap-3 bg-gray-800 border border-gray-700 rounded-lg h-10 pl-1 pr-3 hover:bg-gray-700 transition-colors">
              <img src={user.avatarUrl} alt={user.name} className="w-8 h-8 rounded-full object-cover" />
              <div className="text-sm text-left">
                  <p className="font-semibold text-white leading-tight">{user.name}</p>
                  <span className={`text-xs font-medium ${roleStyle} leading-tight`}>{translatedRole}</span>
              </div>
          </button>
